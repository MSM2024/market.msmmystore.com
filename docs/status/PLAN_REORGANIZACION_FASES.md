...
| 1.2 | **Endurecer API auth** (P0): auth+rol en `api/biblioteca/*` (8 rutas), `api/knowledge/seed`, `api/stripe/{checkout,billing,portal}` (verificar sesión real, no `userId` del body), `api/eliana/health`. ✅ **Código 2026-07-31** (`src/lib/api-auth.ts` + `BibliotecaRepository` con cliente server per-request + gates en las 13 rutas). ⏳ Pendiente validación e2e con claves reales. | P0 | Pruebas: petición sin token → 401; con rol no autorizado → 403. |
